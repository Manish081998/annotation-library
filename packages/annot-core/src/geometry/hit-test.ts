 ...
 */
export function hitTestHandle(p: Point, r: Rect, radius = HANDLE_HIT_RADIUS): HandleIndex | null {
 ...
 */
export function hitTestAnnotation(p: Point, a: Annotation, tol = DEFAULT_HIT_TOLERANCE): boolean {
...
function hitTestRectEdge(p: Point, r: Rect, tol: number): boolean {
...
export function getAnnotationBounds(a: Annotation): Rect | null {
  const { geometry } = a;
  switch (geometry.kind) {
 ...
 */
export function selectionBounds(annotations: ReadonlyArray<Annotation>): Rect | null {
  if (annotations.length === 0) return null;
  const bounds = annotations.map(getAnnotationBounds).filter((b): b is Rect => b !== null);
