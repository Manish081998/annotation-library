...
export function scalePt(p: Point, s: number): Point {
...
export function clampPt(p: Point, bounds: Rect): Point {
  return {
...
export function pointAngleDeg(from: Point, to: Point): number {
 ...
 */
export function rectResizeByHandle(
  original: Rect,
  handle: HandleIndex,
  delta: Point
): Rect {
  ...
  switch (handle) {
    case 0: x += dx; y += dy; width -= dx; height -= dy; break; // TL
    case 1:           y += dy;              height -= dy; break; // TC
    case 2:           y += dy; width += dx; height -= dy; break; // TR
    case 3:                    width += dx;               break; // MR
    case 4:                    width += dx; height += dy; break; // BR
    case 5:                                height += dy;  break; // BC
    case 6: x += dx;           width -= dx; height += dy; break; // BL
    case 7: x += dx;           width -= dx;               break; // ML
  }
...
export function rectHandlePoints(r: Rect): [Point, Point, Point, Point, Point, Point, Point, Point] {
...
export function pointsBounds(points: ReadonlyArray<Point>): Rect {
...
export function simplifyPath(points: ReadonlyArray<Point>, tolerance: number): Point[] {
 ...
 */
export function pointToEllipseEdgeDistanceSq(p: Point, r: Rect, samples = 64): number {
  const cx = r.x + r.width / 2;
  const cy = r.y + r.height / 2;
  const rx = r.width / 2;
  const ry = r.height / 2;
