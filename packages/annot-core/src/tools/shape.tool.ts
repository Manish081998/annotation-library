...
import type { Point, Rect, AnnotationGeometry } from '../model/annotation.model.js';
...
export class ShapeTool implements Tool {
  ...
  constructor(public readonly id: ShapeToolId) {}
  ...
  onPointerUp(evt: ToolPointerEvent, ctx: ToolContext): Command | null {
    ...
    const annotation = buildAnnotation(
      ctx.docId, ctx.pageIndex, this.id, ctx.author, style, geometry
    );
  ...
  drawOverlay(ctx2d: CanvasRenderingContext2D, ctx: ToolContext): void {
    ...
    switch (this.id) {
      ...
      case 'highlight': {
        ...
        if (this.id === 'rectangle') ctx2d.strokeRect(rx, ry, rw, rh);
        break;
      }
      case 'ellipse': {
  ...
  private drawArrowHead(ctx2d: CanvasRenderingContext2D, from: Point, to: Point): void {
    const headLen = 14;
  ...
  private buildGeometry(start: Point, end: Point): AnnotationGeometry {
    switch (this.id) {
      case 'rectangle':
      case 'highlight':
      case 'ellipse':
