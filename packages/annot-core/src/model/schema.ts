...
function validatePoint(v: unknown, path: string, errors: string[]): void {
  if (!isObject(v)) { errors.push(`${path}: expected object`); return; }
  if (!isNumber(v['x'])) errors.push(`${path}.x: expected number`);
  if (!isNumber(v['y'])) errors.push(`${path}.y: expected number`);
}

function validateRect(v: unknown, path: string, errors: string[]): void {
  if (!isObject(v)) { errors.push(`${path}: expected object`); return; }
  if (!isNumber(v['x'])) errors.push(`${path}.x: expected number`);
  if (!isNumber(v['y'])) errors.push(`${path}.y: expected number`);
  if (!isNumber(v['width']) || (v['width'] as number) < 0) errors.push(`${path}.width: expected non-negative number`);
  if (!isNumber(v['height']) || (v['height'] as number) < 0) errors.push(`${path}.height: expected non-negative number`);
}

function validateStyle(v: unknown, errors: string[]): void {
  const path = 'style';
  if (!isObject(v)) { errors.push(`${path}: expected object`); return; }
  if (!isString(v['strokeColor'])) errors.push(`${path}.strokeColor: expected string`);
  if (!isString(v['fillColor'])) errors.push(`${path}.fillColor: expected string`);
  if (!isNumber(v['opacity']) || (v['opacity'] as number) < 0 || (v['opacity'] as number) > 1) {
    errors.push(`${path}.opacity: expected number in [0,1]`);
  }
  if (!isNumber(v['strokeWidth']) || (v['strokeWidth'] as number) < 0) {
    errors.push(`${path}.strokeWidth: expected non-negative number`);
  }
}

function validateGeometry(v: unknown, errors: string[]): void {
  const path = 'geometry';
  if (!isObject(v)) { errors.push(`${path}: expected object`); return; }
  const kind = v['kind'];
  if (kind === 'rect') {
    ...
    if (!isArray(v['points'])) {
      ...
      (v['points'] as unknown[]).forEach((p, i) => validatePoint(p, `${path}.points[${i}]`, errors));
    }
    if (!isBoolean(v['closed'])) errors.push(`${path}.closed: expected boolean`);
    ...
    if (!isBoolean(v['hasArrowHead'])) errors.push(`${path}.hasArrowHead: expected boolean`);
    if (!isBoolean(v['hasArrowTail'])) errors.push(`${path}.hasArrowTail: expected boolean`);
  } else {
...
export function validateAnnotation(v: unknown): ValidationResult {
  const errors: string[] = [];
  if (!isObject(v)) {
    return { valid: false, errors: ['root: expected object'] };
  }
  if (!isNumber(v['schemaVersion'])) errors.push('schemaVersion: expected number');
  if (!isString(v['id'])) errors.push('id: expected string');
  if (!isString(v['docId']) || !(v['docId'] as string).length) errors.push('docId: expected non-empty string');
  if (!isNumber(v['pageIndex']) || (v['pageIndex'] as number) < 0) errors.push('pageIndex: expected non-negative integer');
  if (!isString(v['type']) || !VALID_TYPES.has(v['type'] as string)) errors.push(`type: must be one of ${[...VALID_TYPES].join(', ')}`);
  if (!isString(v['author'])) errors.push('author: expected string');
  if (!isString(v['createdAt'])) errors.push('createdAt: expected string');
  if (!isString(v['updatedAt'])) errors.push('updatedAt: expected string');
  if (!isBoolean(v['isLocked'])) errors.push('isLocked: expected boolean');
  validateStyle(v['style'], errors);
  validateGeometry(v['geometry'], errors);
  if (!isObject(v['meta'])) errors.push('meta: expected object');
  return { valid: errors.length === 0, errors };
}

/** Validate an AnnotationDocument envelope */
export function validateAnnotationDocument(v: unknown): ValidationResult {
  const errors: string[] = [];
  if (!isObject(v)) {
    return { valid: false, errors: ['root: expected object'] };
  }
  if (!isNumber(v['schemaVersion'])) errors.push('schemaVersion: expected number');
  if (v['schemaVersion'] as number > CURRENT_SCHEMA_VERSION) {
    errors.push(`schemaVersion: document version ${String(v['schemaVersion'])} > library version ${CURRENT_SCHEMA_VERSION}`);
  }
  if (!isString(v['docId'])) errors.push('docId: expected string');
  if (!isString(v['exportedAt'])) errors.push('exportedAt: expected string');
  if (!isArray(v['annotations'])) {
    errors.push('annotations: expected array');
  } else {
    (v['annotations'] as unknown[]).forEach((a, i) => {
